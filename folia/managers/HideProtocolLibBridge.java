...
final class HideProtocolLibBridge implements HidePacketBridge {
    ...
    public void refresh(Player target) {
        ...
        plugin.getFoliaScheduler().runEntity(target, () -> syncNametagDisplay(target));
    ...
    private WrappedGameProfile profileFor(Player viewer, Player target) {
        ...
        HideManager.HeadTexture texture = profileTexture(
                state, target.getUniqueId(), bypass);
    ...
    private HideManager.HeadTexture profileTexture(
            HideState state, UUID profileId, boolean bypass) {
        if (state == null) return null;
        if (bypass) {
            HideManager.HeadTexture original = hideManager.originalSkinTexture(profileId);
            if (original != null && original.isValid()) return original;
            if (state.mode() == HideMode.SCRAMBLE && state.hasTexture()) {
                return new HideManager.HeadTexture(state.textureValue(), state.textureSignature());
            }
            return null;
        }
        return state.hasTexture()
                ? new HideManager.HeadTexture(state.textureValue(), state.textureSignature())
                : null;
    }
