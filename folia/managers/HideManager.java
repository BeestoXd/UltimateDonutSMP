...
public class HideManager {
    ...
    public void disguise(Player player, String aliasInput, String skinInput, Consumer<Result> completion) {
        disguise(player, aliasInput, skinInput, false, completion);
    }

    public void disguiseWithScrambledAlias(Player player, String skinInput, Consumer<Result> completion) {
        ...
        plugin.getFoliaScheduler().runAsync(() -> {
            ...
            plugin.getFoliaScheduler().runEntity(player, () -> {
                ...
                for (int attempt = 0; attempt < attempts; attempt++) {
                    ...
                    HideState state = new HideState(
                            player.getUniqueId(), player.getName(), HideMode.DISGUISE,
                            alias, normalize(alias), skinKey, source,
                            texture.value(), texture.signature(),
    ...
    private void resolveSkinAsync(HideState requestedState) {
        plugin.getFoliaScheduler().runAsync(() -> {
            ...
            if (online != null) {
                plugin.getFoliaScheduler().runEntity(online, () -> applyDisguiseSkin(online, updated));
            }
        });
    }
    ...
    private void resolveScrambleSkinAsync(HideState requestedState) {
        ...
        if (requestedState == null || tablist == null) return;
        plugin.getFoliaScheduler().runAsync(() -> {
            TablistManager.SkinTexture resolved = tablist.resolveOriginalGameProfileSkinTexture(
                    requestedState.playerUuid(), requestedState.realNameSnapshot());
            if (resolved == null || !resolved.isValid()) return;
            Player online = Bukkit.getPlayer(requestedState.playerUuid());
            if (online != null) {
                plugin.getFoliaScheduler().runEntity(
                        online, () -> applyResolvedScrambleTexture(requestedState, resolved));
            }
        });
    }

    private void applyResolvedScrambleTexture(
            HideState requestedState, TablistManager.SkinTexture resolved) {
        ...
        HideState updated = current.withTexture(
                resolved.value(), resolved.signature(), System.currentTimeMillis());
        if (!plugin.getDatabaseManager().saveHideState(updated)) return;
        cache(updated);
        HeadTexture texture = new HeadTexture(resolved.value(), resolved.signature());
        originalSkinTextures.put(updated.playerUuid(), texture);
        Player online = Bukkit.getPlayer(updated.playerUuid());
        if (online != null) applySkinTexture(online, texture);
    }
    ...
    private HeadTexture resolveCustomSkin(String source) {
        ...
        return isSkinUrl(source) ? null : resolveHeadTexture(source);
    }

    private HideState ensureScrambleTexture(Player player, HideState state) {
        if (state == null || state.mode() != HideMode.SCRAMBLE) return state;
        if (state.hasTexture()) {
            originalSkinTextures.putIfAbsent(
                    state.playerUuid(), new HeadTexture(state.textureValue(), state.textureSignature()));
            return state;
        }
        ...
        HideState updated = state.withTexture(
                original.value(), original.signature(), System.currentTimeMillis());
        if (plugin.getDatabaseManager().saveHideState(updated)) {
            cache(updated);
            return updated;
        }
    ...
    private HeadTexture rememberOriginalSkin(Player player) {
        ...
        TablistManager tablist = plugin.getTablistManager();
        TablistManager.SkinTexture texture = tablist == null ? null : tablist.resolveLiveGameProfileSkinTexture(player);
    ...
    private void applyDisguiseSkin(Player player, HideState state) {
        if (player == null || state == null || !state.hasTexture()) return;
        boolean applied = applyWithSkinsRestorer(player, state.textureValue(), state.textureSignature());
        if (!applied) {
            try {
                applied = NativeGameProfileFactory.applyTexture(
                        WrappedGameProfile.fromPlayer(player), state.textureValue(), state.textureSignature());
        ...
        refresh(player);
        if (!applied) plugin.getLogger().fine("Disguise texture could not be applied directly to " + player.getName() + ".");
    }
    ...
    private void applySkinTexture(Player player, HeadTexture texture) {
        if (player == null || texture == null || !texture.isValid()) return;
        boolean applied = applyWithSkinsRestorer(player, texture.value(), texture.signature());
        if (!applied) {
            try {
                NativeGameProfileFactory.applyTexture(
                        WrappedGameProfile.fromPlayer(player), texture.value(), texture.signature());
            } catch (RuntimeException | LinkageError ignored) {
            }
        }
