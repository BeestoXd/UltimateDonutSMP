...
public class DuelWorldManager {
    ...
    public void ensureFlatPool() {
        ...
        flatPoolTask = plugin.getFoliaScheduler().runGlobalTimer(this::tickFlatPool, 1L, periodTicks);
    }
    ...
    public void ensureVanillaPool() {
        ...
        vanillaPoolTask = plugin.getFoliaScheduler().runGlobalTimer(this::tickVanillaPool, 1L, periodTicks);
    }
    ...
    private boolean tryLoadPreparationChunkAsync(VanillaPreparation preparation, World world, ChunkCoord chunk) {
        ...
        future.whenComplete((loadedChunk, throwable) -> plugin.getFoliaScheduler().runGlobal(() -> {
    ...
    private void deleteGeneratedWorld(String worldName) {
        ...
        if (plugin.isEnabled()) {
            plugin.getFoliaScheduler().runAsync(() -> deleteDirectory(folderToDelete));
        } else {
