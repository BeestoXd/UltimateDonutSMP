...
public class ShardTask implements Runnable {
    ...
    public void run() {
        ...
        plugin.getFoliaScheduler().forEachOnlinePlayer((Player player) -> tickPlayer(player, sm));
    }
    ...
    public static void start(UltimateDonutSmp plugin) {
        plugin.getFoliaScheduler().runGlobalTimer(new ShardTask(plugin), 60 * 20L, 60 * 20L); // every minute
    }
}
