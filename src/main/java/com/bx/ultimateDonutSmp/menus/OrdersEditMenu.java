        ...
        if (owner) {
            ...
            if (editable) {
                set(19, ItemUtils.createItem(
                        Material.NAME_TAG,
                        "&bChange item",
                        List.of("&7Use the configured item selection mode.", "", "&eClick to edit")
                ));
                set(20, ItemUtils.createItem(
                        Material.WRITABLE_BOOK,
                        "&eChange quantity",
                        List.of("&7Current: &e" + order.requestedQuantity(), "", "&eClick to edit")
                ));
                set(22, ItemUtils.createItem(
                        Material.GOLD_INGOT,
                        "&eChange price",
                        List.of("&7Current: " + plugin.getCurrencyManager().formatMoney(order.priceEach()), "", "&eClick to edit")
                ));
            } else {
                List<String> lockedLore = order.active()
                        ? List.of("&7This order already has deliveries.")
                        : List.of("&7This order is no longer active.");
